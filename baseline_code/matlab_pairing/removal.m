function removal(seg, segnum, between, label, near, centroids, url, grad, texthist, output_filename)
...
for i = 1:segnum
    ...
    % imshow(test_im);
    % output_filename = 'output_image.jpg'; % 定義輸出的檔名
    % imwrite(test_im, output_filename);    % 保存圖像
    % disp(['Image saved as ', output_filename]);
    % 將圖像保存為 JPEG
    imwrite(test_im, output_filename); 
    disp(['Image saved as ', output_filename]);
end
